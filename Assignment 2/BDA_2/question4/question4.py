...
if __name__ == "__main__":
    sc = SparkContext(appName="Question4")
    ...
    flat_results = result.collect()
    flat_results=list((set(flat_results)))
    flat_results=sc.parallelize(flat_results).sample(False,1).take(10)
    sc.parallelize(flat_results).sample(False,1).saveAsTextFile(sys.argv[1] + sys.argv[4])
