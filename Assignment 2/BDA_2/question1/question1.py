...
if __name__ == "__main__":
    config = SparkConf().setAppName("Question1").setMaster("local[2]")
    ...
    common_friends.map(lambda x:"{0}\t {1} ".format(x[0],len(x[1]))).coalesce(1).saveAsTextFile(sys.argv[1] + sys.argv[3])
